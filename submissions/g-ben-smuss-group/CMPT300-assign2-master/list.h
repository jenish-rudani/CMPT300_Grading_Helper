...
struct Node_s {
    ...
    struct Node_s* prev;
    void* item;
    struct Node_s* _next_free;
};

// List struct holds info about
// how to behave when user calls List_next() or List_prev()
// and reaches beyond the bounds of the list.
enum ListOutOfBounds {
    LIST_OOB_NA,
    LIST_OOB_START, // List_prev() was called when current node was first.
    LIST_OOB_END // List_next() was called when current node was last.
};
...
struct List_s {
    ...
    Node* last;
    Node* current_node;
    struct List_s* _next_free;
    bool OOB_END_flag; // true if beyond last, false if before first.
};
