...
struct Node_s {
    ...
    int nodeArrayIndex;     // Index of where this node is in the NodesArray
    void* itemPtr;          // Pointer to the item that the node contains
    Node* nextNode;         // Node Pointer to the next Node in the List
    Node* prevNode;         // Node Pointer to the prev Node in the list
    int nextFreeNodeIndex;  // Index of where the next free node is
    // Need taken for Nodes that are in memory but with NULL values
    // So it's still in memory because the user hasn't given it values or has taken away its value without freeing the node
    bool taken;  // Whether this Node is currently being used in memory (true) or not (false)
};
...
struct List_s {
    // TODO: You should change this!
    void* currentItemPtr;                       // Pointer to the current Item of list (aka currentNodePtr->itemPtr)
    Node* currentNodePtr;                       // Node Pointer to the current Node of the list
    Node* listHeadNodePtr;                      // Node Pointer to the Head Node of the list
    Node* listTailNodePtr;                      // Node Pointer to the Tail Node of the list
    int totalItemsInList;                       // Integer of the total number of items/Nodes in the list (between Head and Tail)
    int headArrayIndex;                         // Index of where the list (Head node) is in the HeadsArray
    int nextFreeListIndex;                      // Index of where the next free list is
    enum ListOutOfBounds currentNodePtrStatus;  // We only look at this status if the currentItemPtr is NULL
    // taken is used to handle the case if the user creates a lots of lists with empty data. So we use
    // taken as flag to say that this list is using memory depite having no values in it yet
    bool taken;  // This list currently being used in memory if True, False means List is free
};

// Maximum number of unique lists the system can support
// (You may modify this, but reset the value to 10 when handing in your
// assignment)
#define LIST_MAX_NUM_HEADS 10

// Maximum total number of nodes (statically allocated) to be shared across all
// lists (You may modify this, but reset the value to 100 when handing in your
// assignment)
